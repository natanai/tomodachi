...
let goals = [];
let inferredCurrentBandsByPersonality = {};
...
function setupForRegion() {
  ...
  inferredCurrentBandsByPersonality = buildInferredCurrentBands(currentRegion.code, goals);
...
function buildInferredCurrentBands(regionCode, goalList) {
  const bands = {};

  goalList.forEach((goal) => {
    bands[goal.personalityKey] = {
      msMin: goal.msBand[0],
      msMax: goal.msBand[1],
      eaMin: goal.eaBand[0],
      eaMax: goal.eaBand[1]
  ...
  for (const [personalityKey, examples] of Object.entries(regionalExamples)) {
    for (const example of examples) {
      const ms = mappedValue('movement', example.personalitySliders.movement)
        + mappedValue('speech', example.personalitySliders.speech);
      const ea = mappedValue('expressiveness', example.personalitySliders.expressiveness)
        + mappedValue('attitude', example.personalitySliders.attitude);

      if (!bands[personalityKey]) {
        bands[personalityKey] = { msMin: ms, msMax: ms, eaMin: ea, eaMax: ea };
      } else {
        bands[personalityKey].msMin = Math.min(bands[personalityKey].msMin, ms);
        bands[personalityKey].msMax = Math.max(bands[personalityKey].msMax, ms);
        bands[personalityKey].eaMin = Math.min(bands[personalityKey].eaMin, ea);
        bands[personalityKey].eaMax = Math.max(bands[personalityKey].eaMax, ea);
      }
    }
  }
...
function buildMatchesExamplePersonalityAxes(build, example) {
  return PERSONALITY_AXIS_KEYS.every((key) => (
    build[key] === null || example.personalitySliders[key] === build[key]
  ));
}

function hasCompatibleConfirmedExampleForGoal(goal, build, regionCode) {
  const examples = getConfirmedExamplesForGoal(regionCode, goal.personalityKey);
  return examples.some((example) => buildMatchesExamplePersonalityAxes(build, example));
}
...
function isLegacyCompatible(goal, build) {
  const inferredBands = inferredCurrentBandsByPersonality[goal.personalityKey];
  const msMin = inferredBands ? inferredBands.msMin : goal.msBand[0];
  const msMax = inferredBands ? inferredBands.msMax : goal.msBand[1];
  const eaMin = inferredBands ? inferredBands.eaMin : goal.eaBand[0];
  const eaMax = inferredBands ? inferredBands.eaMax : goal.eaBand[1];
    ...
    for (let s = 1; s <= 8; s += 1) {
      ...
      for (let e = 1; e <= 8; e += 1) {
        ...
        for (let a = 1; a <= 8; a += 1) {
          ...
          if (ea >= eaMin && ea <= eaMax) return true;
        }
      }
    }
  }
...
function isAllowedForGoal(key, value, goal) {
  ...
  if (!isCompleteBuild(draft)) {
    if (hasCompatibleConfirmedExampleForGoal(goal, draft, currentRegion.code)) return true;
    return isLegacyCompatible(goal, draft);
  }
